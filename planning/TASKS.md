...
The pledged work backlog. A **proposed** task lives in the
[issue tracker](https://github.com/ferroteca/testaferro/issues),
which is the only queue a proposed task has: the tracker is a task's
proposed state and this file is its pledged one. Nothing parks here
awaiting a verdict; arriving *is* the verdict.
...
`proposed/` and `pledged/` hold *demand and capability* — the use
cases, the principles, and the features that deliver them — each
argued at length before it is pledged. A task is none of those: it
...
it stays at the planning root, in the pledged state, among the
machinery.
...
feature lives with that feature**, in `pledged/FEATURES.md`, and
...
and how a pledge is recorded — is in [README.md](README.md).

**A small interface change may be a task** (D8). Housekeeping's
interface test does not read across to this queue: that boundary
compensates for a class nobody with authority ever reviews, and a
queue only authority can write to needs no such compensation. An
item is admitted here on size and kind, never refused for the
surface it touches. What it may not do is skip the **landing rules**
in [INTERFACES.md](INTERFACES.md), which bind a task exactly as they
bind a feature — that rule governs how a change lands, not which
queue it waited in.
